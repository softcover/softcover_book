...
\PolyTeX\ is a strict subset of the \LaTeX\ typesetting language. This means that not all of \LaTeX\ is supported by Softcover, so to learn \PolyTeX\ I recommend running a local server and trying out commands to see if they work:\footnote{Here we use the \kode{-p} flag to run the server on port 4001 in case another book is already running on port 4000 (the default).}
